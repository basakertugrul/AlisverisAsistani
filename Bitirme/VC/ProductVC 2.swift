...
class ProductVC: UIViewController, UICollectionViewDelegate, UICollectionViewDataSource, CLLocationManagerDelegate {
    ...
    let colorDict: [Int: UIColor] = [1:UIColor.black.withAlphaComponent(0.7), 2:UIColor.white.withAlphaComponent(0.7), 3:UIColor(red: 0.698, green: 0.0784, blue: 0, alpha: 0.7), 4:UIColor(red: 0.898, green: 0.4784, blue: 0, alpha: 0.7), 5:UIColor(red: 0.898, green: 0.8392, blue: 0, alpha: 0.7), 6:UIColor(red: 0.4275, green: 0.7569, blue: 0, alpha: 0.7), 7:UIColor(red: 0, green: 0.5569, blue: 0.698, alpha: 0.7), 8:UIColor(red: 154/255, green: 0/255, blue: 154/255, alpha: 0.7)]
    ...
    @objc func nonExistColorButtonPressed(sender:UIButton) {
        ...
        let params: [String: Any] = ["productNo": String(describing: self.productID),
                                     "color": Int(sender.tag),
                                     "size": self.currentSize]
        NetworkManager.sendlocationofProductRequest(parameters: params)
...
//                vc.locationArray = data
                self!.present(vc, animated: true, completion: nil)
            }
        }
    }
    
    @objc func existSizeButtonPressed(sender:UIButton) {
        ...
        { [weak self] (data, error) in
            ...
            if let dictionary = data {
                ...
                for item in self!.allSizes {
                    ...
                    self!.sizeButtons[i!].setTitleColor(.black, for: .normal)
                    ...
                    if self?.liked == true {
                        self?.likeButton.tintColor = UIColor(red: 0.5882, green: 0, blue: 0.2039, alpha: 1.0)
                    }
                    else{
                        self?.likeButton.tintColor = .black
                    }
