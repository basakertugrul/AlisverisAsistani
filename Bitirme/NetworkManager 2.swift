...
static func sendlocationofProductRequest(parameters: [String: Any],
                               completion: @escaping RequestCompletionBlock) {
    
    let stringToken = String(describing: KeychainWrapper.standard.string(forKey: "token") ?? "" )
    let headers : HTTPHeaders = ["Content-Type": "application/json", "Authorization": "Bearer \(stringToken)"]
    
    guard let productNo = parameters["productNo"] else {return}
    guard let colorint = parameters["color"] else {return}
    let color = String(describing: colorint)
    guard let size = parameters["size"] else {return}
    let url = "http://192.168.1.155:62755/api/store/map/\(productNo))/\(color)/\(size)"
